...
with(df_trb,
  plot_ly(x = year, y = sum_preciptation  , mode = "markers" , color = "blue") %>%
   layout(
...
plot_ly(df, x = df$year, y = df$burn , mode = "markers" , color = as.factor(df$region))

plot_ly(df, x = df$year, y = df$precipitation , mode = "markers" , color = as.factor(df$region))

plot_ly(df, x = df$precipitation, z = df$burn , y = df$year, type = "scatter3d", mode = "markers" , color = as.factor(df$region)) %>%
   layout(
    title = "Forest burn x Cumulative Rainfall",
    scene = list(
      xaxis = list(title = "Precipitation (mm)"),
      zaxis = list(title = "Number of forest burns"),
